...
>({
  ...
  const errorMessage = useMemo<FormItemProps["help"]>(() => {
    let message = getObjectValueWithPath(
      formState.errors,
      replaceLastOccurrence(finalName, ".[]", ".root"),
    );

    if (!message) {
      message = getObjectValueWithPath(
        formState.errors,
        replaceLastOccurrence(finalName, ".[]", ""),
      );
    }
    if (finalName && message) return String(message?.message);
  }, [formState, finalName]);

  const status = useMemo<FormItemProps["validateStatus"]>(() => {
    if (finalName && errorMessage) return "error";
